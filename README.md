...
- [x] Explain the difference between `Relational Databases` and `SQL`.
  ...
  A relational database is a generic term for a database where data is stored in columns where some of the data is linked to each other based on common data. SQL is the language used to communicate with such a database.

- [x] Why do tables need a `primary key`?

  Tables need a primary key so they have something to reference agnostic of the data so that even if the data changes it doesn't become unlinked from the rest of the database.
...
- [x] What do we need in order to have a _many to many_ relationship between two tables.
  ...
  You generally need to have a third table that only stores the relationship between the two.
